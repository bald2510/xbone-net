...
Bài toán được nghiên cứu trong đề tài là bài toán phân loại ảnh X-quang sử dụng học sâu đa phương thức (multimodal deep learning). Mục tiêu của hệ thống là dự đoán tình trạng bệnh hoặc tổn thương dựa trên đồng thời dữ liệu hình ảnh X-quang và thông tin văn bản lâm sàng liên quan.
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    \item Thông tin văn bản lâm sàng:
    \[
        x^{txt} = \{w_1, w_2, ..., w_T\}
    \]
    trong đó:
  ...
  \begin{itemize}
    \item Phân phối xác suất trên tập nhãn:
    \[
        \hat{y} = \text{Softmax}(Wz + b)
    \]
    trong đó:
    \begin{itemize}
        \item $W \in \mathrm{R}^{K \times d}$ là ma trận trọng số,
        \item $b \in \mathrm{R}^{K}$ là vector bias,
        \item $\hat{y} \in \mathrm{R}^{K}$ là phân phối xác suất dự đoán.
    \end{itemize}

    \item Nhãn dự đoán cuối cùng:
    \[
        y^{*} = \arg\max_{k \in \{1,...,K\}} \hat{y}_k
    \]

    \item Các bản đồ trực quan hóa trên ảnh và văn bản nhằm giải thích quyết định của mô hình:
    \[
        \mathcal{A}^{img}
    \]
    trong đó:
    \begin{itemize}
        \item $\mathcal{A}^{img}$ biểu diễn vùng ảnh quan trọng,
    \end{itemize}
  \end{itemize}
\end{itemize}
