...
Bài toán phát hiện dữ liệu ngoài phân phối (OOD detection) đóng vai trò then chốt trong việc đảm bảo tính an toàn và độ tin cậy của các hệ thống trí tuệ nhân tạo, đặc biệt trong các ứng dụng rủi ro cao như chẩn đoán hình ảnh y khoa. Dựa trên cơ chế hoạt động và cách biểu diễn dữ liệu, các phương pháp OOD hiện nay có thể được phân loại thành các nhóm chính sau đây.
\subsection{Phương pháp dựa trên đầu ra của mô hình (Post-hoc Methods)}
Nhóm phương pháp này tận dụng trực tiếp đầu ra (logits hoặc probabilities) của một mạng nơ-ron đã được huấn luyện mà không cần thay đổi kiến trúc hay huấn luyện lại mô hình.
\begin{itemize}
  \item \textbf{Maximum Softmax Probability (MSP):}
Đây là phương pháp nền tảng, dựa trên giả định rằng mô hình sẽ đưa ra xác suất dự đoán thấp hơn đối với các mẫu OOD. Tuy nhiên, các mạng nơ-ron sâu thường gặp hiện tượng "tự tin thái quá" (overconfidence), khiến MSP kém hiệu quả trên thực tế.
  \item \textbf{ODIN (Out-of-DIstribution detector for Neural networks):}
Khắc phục nhược điểm của MSP bằng cách sử dụng kỹ thuật Temperature Scaling và thêm nhiễu (perturbation) vào đầu vào. Cơ chế này giúp khuếch đại sự khác biệt về logit giữa mẫu In-Distribution (ID) và OOD.
  \item \textbf{Energy-based Scores:} Thay vì sử dụng xác suất phân bố, phương pháp này
chuyển đổi các giá trị logit thành mức năng lượng. Các mẫu ID sẽ có mức năng lượng thấp, trong khi các mẫu OOD có năng lượng cao. Hàm năng lượng được định nghĩa như sau: $$E(x; f) = -T \log \sum_{i} \exp(f_i(x)/T)$$ Trong đó, $f_i(x)$ là logit của lớp $i$ và $T$ là tham số nhiệt độ (temperature).
\end{itemize}

\subsection{Không gian đặc trưng và Mạng nguyên mẫu }
Thay vì chỉ sử dụng lớp đầu ra, các phương pháp này khai thác không gian ẩn (latent space) để tính toán khoảng cách từ mẫu kiểm thử đến phân phối dữ liệu huấn luyện. Tiêu biểu trong nhóm này là kiến trúc \textbf{Mạng nguyên mẫu (Prototypical Networks)}. Trong không gian nhúng, các mẫu của cùng một lớp được gom cụm xung quanh một "nguyên mẫu" (prototype) $c_k$, được tính bằng trung bình các vector đặc trưng của tập dữ liệu hỗ trợ $S_k$:$$c_k = \frac{1}{|S_k|} \sum_{x_i \in S_k} f_\theta(x_i)$$ Khả năng phát hiện OOD của phương pháp này phụ thuộc chặt chẽ vào độ đo khoảng cách được sử dụng:
\begin{itemize}
  \item \textbf{Khoảng cách Euclidean ($L_2$ Norm):} Tính toán bằng công thức $d_E(z, c_k) = \| z - c_k \|_2$.
Phương pháp này giả định các cụm dữ liệu có dạng hình cầu và đẳng hướng (isotropic). Dù dễ tính toán, nó thường kém hiệu quả do bỏ qua sự tương quan và độ phương sai khác biệt giữa các chiều đặc trưng.
  \item \textbf{Khoảng cách Cosine:} Định lượng góc giữa hai vector ($d_C(z, c_k) = 1 - \frac{z \cdot c_k}{\|z\|_2 \|c_k\|_2}$).
Khoảng cách này loại bỏ ảnh hưởng của độ lớn vector (magnitude), rất hiệu quả khi kết hợp với các hàm mất mát đối chiếu (Contrastive Loss), nhưng có thể làm mất đi tín hiệu nhận diện quan trọng khi các mẫu OOD thường có norm thấp hơn ID.
  \item \textbf{Khoảng cách Mahalanobis:} Là độ đo mạnh mẽ nhất cho Near-OOD, được biểu diễn qua công thức
$d_M(z, c_k) = \sqrt{(z - c_k)^T \Sigma_k^{-1} (z - c_k)}$ (với $\Sigma_k$ là ma trận hiệp phương sai). Bằng cách cho phép các cụm dữ liệu có dạng ellipsoid linh hoạt (anisotropic), Mahalanobis đánh giá được mức độ bất thường dựa trên chính sự phân bố nội bộ của lớp, dù chi phí tính toán nghịch đảo ma trận là một rào cản lớn khi tập dữ liệu hỗ trợ nhỏ.
\end{itemize}
...
được huấn luyện chỉ trên dữ liệu ID (ví dụ: các cấu trúc giải phẫu bình thường). Khi gặp dữ liệu OOD, mô hình sẽ không thể khôi phục chính xác, dẫn đến lỗi tái tạo pixel-level cao, từ đó không chỉ phát hiện mà còn khoanh vùng được vùng bất thường.\item \textbf{Monte Carlo Dropout (MC Dropout):} Để định lượng "độ không chắc chắn nhận thức" (epistemic uncertainty), kỹ thuật này thực hiện quá trình truyền xuôi (forward pass) nhiều lần với dropout được kích hoạt trong pha kiểm thử. Phương sai lớn giữa các lần dự đoán là tín hiệu mạnh mẽ chỉ ra rằng mô hình đang đối mặt với dữ liệu chưa từng quan sát.
\end{itemize}

\subsection{Học sâu Đa phương thức trong phát hiện OOD Y khoa (Multimodal Deep Learning)}
Trong bối cảnh phân loại ảnh y khoa phức tạp, ranh giới giữa ID và OOD thường rất mỏng manh (Near-OOD) do sự đa dạng của các bệnh lý hiếm gặp hoặc hiện tượng dịch chuyển phân phối (Covariate shift). Các phương pháp OOD hiện đại đang dịch chuyển mạnh mẽ sang kiến trúc Học sâu đa phương thức (Multimodal Deep Learning - MDL).
\begin{itemize}
  \item \textbf{Supervised Contrastive Learning:} Nhằm giải quyết Covariate shift giữa các hệ thống thu nhận hình ảnh
(ví dụ: các máy chụp X-quang khác nhau), kỹ thuật đối chiếu có giám sát ép mô hình học các biểu diễn hình thái cốt lõi của tổn thương thay vì ghi nhớ nhiễu (noise/texture). Điều này giúp tối ưu hóa ranh giới khoảng cách trong không gian ẩn.
  \item \textbf{Zero-shot OOD với Vision-Language Models:} Sự xuất hiện của các mô hình nền tảng y khoa như BiomedCLIP
cho phép phát hiện OOD thông qua độ lệch pha (misalignment) giữa đặc trưng hình ảnh và văn bản. Cụ thể, thông qua cơ chế Knowledge-guided Cross-Attention, hệ thống có thể đối chiếu chéo (cross-reference) các vùng hình ảnh với kho tri thức y khoa. Ví dụ, trong bài toán phân loại khối u xương (bone tumor) đa phương thức, nếu mô hình đối mặt với một ảnh X-quang chứa mảnh ghép kim loại chỉnh hình hoặc một dạng Sarcoma chưa từng có trong tập huấn luyện, điểm tương đồng (Alignment Score) giữa đặc trưng ảnh và các báo cáo u xương tiêu chuẩn sẽ suy giảm đột ngột. Sự sụt giảm độ chú ý tại các vùng bất thường này cung cấp cơ chế "cắm cờ" (flag) OOD mạnh mẽ và có khả năng giải thích cao (interpretable).
\end{itemize}
