...
Ngoài nhiệm vụ chính của hệ thống, khóa luận bổ sung cơ chế phát hiện dữ liệu ngoài phân phối ở giai đoạn hậu xử lý mà không yêu cầu mẫu ngoài phân phối trong quá trình huấn luyện. Thay vì ước lượng trung bình lớp và ma trận hiệp phương sai như trong khoảng cách Mahalanobis yêu cầu một phân phối dữ liệu không đảm bảo đạt được khi triển trai, phương pháp sử dụng kNN trực tiếp trên các embedding đầu ra của hai bộ mã hóa BiomedCLIP. Cụ thể, embedding ảnh và embedding văn bản của tập huấn luyện được lưu thành hai ngân hàng tham chiếu độc lập, ký hiệu lần lượt là $\mathcal{B}_{I}$ và $\mathcal{B}_{T}$. Với một mẫu mới, embedding ảnh $z_I$ và embedding văn bản $z_T$ được đối sánh riêng với ngân hàng tương ứng để xác định các láng giềng gần nhất và thu được tín hiệu khoảng cách lân cận cho từng phương thức. Các tín hiệu này phản ánh mức độ gần của mẫu mới đối với những biểu diễn đã quan sát trong tập huấn luyện và được sử dụng để hình thành cảnh báo ngoài phân phối theo ngưỡng hiệu chỉnh trên tập xác thực. Trong phạm vi khóa luận, cảnh báo ngoài phân phối đóng vai trò tín hiệu bổ trợ; chi tiết cách tính điểm từ các láng giềng và quy tắc sử dụng hai tín hiệu ảnh-văn bản được trình bày trong Chương~\ref{Chapter3}.
...
\begin{itemize}
    ...
    các bộ mã hóa tiếp tục được thích nghi thông qua LoRA, đồng thời mô-đun xử lý đặc trưng cục bộ và cơ chế chú ý chéo hai chiều được tối ưu cho nhiệm vụ phân lớp. Sau khi quá trình thích nghi hoàn tất, embedding ảnh và embedding văn bản của tập huấn luyện được trích xuất từ hai bộ mã hóa để xây dựng hai ngân hàng tham chiếu phục vụ kNN trong giai đoạn suy luận.
\end{itemize}
