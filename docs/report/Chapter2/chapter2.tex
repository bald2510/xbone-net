...
Sự phát triển của các mô hình ngôn ngữ lớn (Large Language Models - LLMs) đã thúc đẩy nhiều nghiên cứu trong lĩnh vực y tế, đặc biệt là các mô hình đa phương thức có khả năng kết hợp thông tin hình ảnh và văn bản để hỗ trợ chẩn đoán và trả lời câu hỏi y khoa.

Một số mô hình tiêu biểu bao gồm Med-Flamingo \cite{moor_2023_medflamingo}, mô hình học ít mẫu (few-shot) đầu tiên dành cho y tế; LLaVA-Med \cite{li_2023_llavamed}, mở rộng khả năng thực hiện theo chỉ dẫn (instruction-following) sang miền y sinh thông qua dữ liệu hội thoại được xây dựng từ các cặp ảnh–văn bản; Med-PaLM M \cite{MED-PALM-M}, mô hình đa nhiệm và đa phương thức quy mô lớn của Google; và SkinGPT-4 \cite{SkinGPT-4}, hệ thống hỗ trợ chẩn đoán da liễu kết hợp khả năng hiểu ảnh y khoa với suy luận ngôn ngữ.

Mặc dù đạt hiệu quả cao trong nhiều tác vụ như trả lời câu hỏi y khoa, hội thoại và hỗ trợ ra quyết định, các mô hình này thường có quy mô rất lớn, chi phí huấn luyện và suy luận cao, đồng thời vẫn tồn tại nguy cơ tạo ra thông tin không chính xác (hallucination). Do đó, đối với các bài toán chuyên biệt như phân loại ảnh X-quang, việc sử dụng các mô hình thị giác hoặc mô hình ngôn ngữ–thị giác chuyên dụng thường phù hợp và hiệu quả hơn.
...
\subsection{Các phương pháp hậu xử lý dựa trên đầu ra mô hình (Post-hoc Methods)} 
...
\subsection{Các phương pháp dựa trên không gian đặc trưng (Feature-space-based Methods)} 
Khác với các phương pháp chỉ sử dụng đầu ra của mô hình, nhóm phương pháp này thực hiện phát hiện OOD trực tiếp trên không gian đặc trưng (embedding space). Ý tưởng chính là đo khoảng cách giữa embedding của mẫu kiểm thử và phân bố đặc trưng của dữ liệu huấn luyện. Hiệu quả của nhóm phương pháp này phụ thuộc mạnh vào chất lượng biểu diễn đặc trưng mà mô hình học được. Một số độ đo khoảng cách phổ biến bao gồm: 
\begin{itemize} 
...
\subsection{Các phương pháp học biểu diễn phục vụ phát hiện OOD\\(Representation Learning)}
...
\subsection{Các phương pháp sinh và định lượng độ không chắc chắn} Nhóm phương pháp này tiếp cận bài toán OOD thông qua khả năng mô hình hóa phân phối dữ liệu hoặc ước lượng độ không chắc chắn của mô hình. 
\begin{itemize} 
...
\subsection{Cơ sở lựa chọn phương pháp} Các phương pháp OOD hiện nay đều có những ưu điểm và hạn chế riêng. Các phương pháp hậu xử lý dựa trên đầu ra như MSP, ODIN và Energy Score có ưu điểm là đơn giản nhưng hiệu quả phụ thuộc nhiều vào đầu ra của bộ phân loại và thường suy giảm đối với các trường hợp Near-OOD. Trong khi đó, các phương pháp sinh và định lượng độ không chắc chắn có chi phí tính toán cao và yêu cầu kiến trúc mô hình phức tạp hơn. Đối với các phương pháp dựa trên khoảng cách, hiệu quả phát hiện OOD phụ thuộc trực tiếp vào chất lượng của không gian đặc trưng. Nếu embedding của các lớp chưa đủ tập trung hoặc có phương sai nội lớp lớn thì việc sử dụng các độ đo khoảng cách sẽ khó phân biệt được dữ liệu ID và OOD. Vì vậy, nghiên cứu này lựa chọn kết hợp \textbf{Prototypical Loss} và \textbf{Mahalanobis Distance}. Trong giai đoạn huấn luyện, Prototypical Loss được sử dụng để học không gian đặc trưng có tính phân biệt cao thông qua việc giảm phương sai nội lớp và tăng khoảng cách giữa các lớp. Sau khi mô hình được huấn luyện, Mahalanobis Distance được áp dụng theo cơ chế \textit{post-hoc} để đo khoảng cách từ embedding của mẫu kiểm thử đến phân bố đặc trưng của từng lớp. Mặc dù việc phân loại được thực hiện dựa trên độ tương đồng Cosine giữa embedding và prototype, Mahalanobis vẫn được sử dụng trong giai đoạn phát hiện OOD vì có khả năng khai thác thêm thông tin về ma trận hiệp phương sai của từng lớp, từ đó phản ánh chính xác hơn mức độ bất thường của mẫu trong không gian đặc trưng. Sự kết hợp này tận dụng được ưu điểm của cả hai thành phần: Prototypical Loss giúp xây dựng embedding có tính phân biệt cao, trong khi Mahalanobis Distance cung cấp một cơ chế phát hiện OOD hiệu quả mà không cần huấn luyện thêm mô hình chuyên biệt.
...
Song song với đó, sự xuất hiện của các mô hình ngôn ngữ trực quan y khoa (Medical Vision-Language Models) như BioMedCLIP đã mở ra hướng tiếp cận mới cho các bài toán phân loại ảnh y khoa. Nhờ được tiền huấn luyện trên tập dữ liệu ảnh -- văn bản quy mô lớn, các mô hình này học được biểu diễn đa phương thức giàu ngữ nghĩa và có khả năng thích nghi tốt với nhiều tác vụ khác nhau. Kết hợp với các phương pháp tinh chỉnh hiệu quả tham số (Parameter-Efficient Fine-Tuning - PEFT), việc khai thác các mô hình nền tảng trở nên khả thi hơn ngay cả khi dữ liệu huấn luyện chuyên ngành còn hạn chế.
...
\subsection{Cơ sở lựa chọn phương pháp đề xuất}

Từ những hạn chế trên, luận văn lựa chọn xây dựng hệ thống dựa trên mô hình BioMedCLIP kết hợp với thông tin lâm sàng để khai thác đồng thời đặc trưng hình ảnh và văn bản. Thay vì tinh chỉnh toàn bộ mô hình, kỹ thuật LoRA được sử dụng nhằm giảm số lượng tham số cần cập nhật, từ đó giảm chi phí huấn luyện trong khi vẫn bảo tồn phần lớn tri thức đã được học từ giai đoạn tiền huấn luyện.

Để khai thác hiệu quả hai nguồn dữ liệu, đặc trưng hình ảnh và đặc trưng văn bản được kết hợp thông qua mô-đun dung hợp đa phương thức trước khi thực hiện phân loại. Cách tiếp cận này giúp mô hình học được mối quan hệ bổ sung giữa hai phương thức dữ liệu, phù hợp với quy trình chẩn đoán thực tế của bác sĩ.

Bên cạnh mục tiêu phân loại, luận văn sử dụng Prototypical Loss để học không gian đặc trưng có tính phân biệt cao hơn. Hàm mất mát này không trực tiếp thực hiện phát hiện dữ liệu ngoài phân phối mà đóng vai trò giảm phương sai nội lớp và tăng khoảng cách giữa các lớp trong không gian embedding, tạo ra các biểu diễn đặc trưng ổn định và dễ phân tách hơn.

Trên không gian đặc trưng đã học, khoảng cách Mahalanobis được sử dụng theo cơ chế \textit{post-hoc} để phát hiện dữ liệu ngoài phân phối. Khác với khoảng cách Euclidean hoặc Cosine chỉ phản ánh mức độ tương đồng giữa các vector đặc trưng, Mahalanobis còn khai thác thông tin về phương sai và mối tương quan giữa các chiều đặc trưng của từng lớp. Điều này giúp mô hình đánh giá chính xác hơn mức độ bất thường của mẫu kiểm thử, đặc biệt đối với các trường hợp Near-OOD thường gặp trong dữ liệu y khoa.

Sự kết hợp giữa Prototypical Loss và Mahalanobis Distance cho phép tách biệt hai giai đoạn của hệ thống: Prototypical Loss được sử dụng trong quá trình huấn luyện để xây dựng không gian đặc trưng có tính phân biệt cao, trong khi Mahalanobis Distance được áp dụng sau khi huấn luyện để phát hiện dữ liệu ngoài phân phối mà không cần xây dựng thêm một mô hình OOD chuyên biệt.

\subsection{Định hướng của luận văn}

Từ các phân tích trên, luận văn đề xuất một hệ thống phân loại ảnh X-quang xương đa phương thức dựa trên BioMedCLIP kết hợp với thông tin lâm sàng. Hệ thống sử dụng LoRA để tinh chỉnh mô hình nền, mô-đun dung hợp đa phương thức để kết hợp đặc trưng ảnh và văn bản, Prototypical Loss để học không gian đặc trưng có tính phân biệt và Mahalanobis Distance để phát hiện dữ liệu ngoài phân phối theo cơ chế \textit{post-hoc}. Kiến trúc của hệ thống cùng quy trình huấn luyện và đánh giá sẽ được trình bày chi tiết trong Chương 3.
