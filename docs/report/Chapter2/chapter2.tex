...
Ảnh y khoa thường có độ phân giải cao để ghi nhận được các dấu hiệu bệnh lý nhỏ, trong khi các bộ mã hóa thị giác tiền huấn luyện thường yêu cầu kích thước đầu vào cố định ở kích thước nhỏ. Việc co toàn bộ ảnh về kích thước nhỏ giúp giảm chi phí tính toán nhưng có thể làm mất các chi tiết cục bộ. Ngược lại, xử lý trực tiếp ảnh độ phân giải cao làm tăng bộ nhớ cần sử dụng và chi phí tính toán cao của cơ chế chú ý. Bảng \ref{tab:ch2_high_resolution} tổng hợp về các chiến lược xử lý ảnh độ phân giải cao và sự đánh đổi chính được trình bày. FPT \cite{FPT} giải quyết sự đánh đổi này bằng cách lựa chọn một phần đơn vị biểu diễn kết hợp với mạng nhánh, cho thấy việc thích nghi mô hình nền tảng cho ảnh y khoa cần đồng thời bảo toàn chi tiết và kiểm soát bộ nhớ.

Trong các mô hình ngôn ngữ trực quan tổng quát, LLaVA-NeXT \cite{LLaVANeXTAblations2024} đề xuất AnyRes, trong đó ảnh được biểu diễn bởi một ảnh toàn cục và nhiều ô ảnh cục bộ theo lưới thích nghi. Phiên bản Higher-AnyRes mở rộng số ô ảnh để hỗ trợ đầu vào có độ phân giải lớn hơn, đồng thời giới hạn tổng số đơn vị biểu diễn bằng phép nội suy đặc trưng. Các thí nghiệm cho thấy tăng độ phân giải quan sát thường cải thiện các tác vụ phụ thuộc vào chi tiết, trong khi tăng mạnh số đơn vị biểu diễn hoặc phóng lớn ảnh một cách cưỡng bức làm tăng chi phí nhưng không luôn đem lại hiệu quả tương ứng. Kết quả này nhấn mạnh rằng mục tiêu chính là giữ lại thông tin cục bộ hữu ích trong một ngân sách đơn vị biểu diễn phù hợp.
...
\multicolumn{1}{|c|}{\textbf{Chiến lược}} &
  \multicolumn{1}{c|}{\textbf{Đặc điểm}} &
  \multicolumn{1}{c|}{\textbf{Khả năng đáp ứng}} &
  \multicolumn{1}{c|}{\textbf{Hạn chế cần xem xét}} \\ \hline
Co giãn ảnh toàn cục &
  \begin{tabular}[c]{@{}l@{}}Một ảnh được đưa về \\ kích thước cố định\end{tabular} &
  \begin{tabular}[c]{@{}l@{}}Chi phí thấp và duy \\ trì bố cục tổng thể\end{tabular} &
  \begin{tabular}[c]{@{}l@{}}Có thể làm mất các \\ dấu hiệu nhỏ hoặc \\ khu trú.\end{tabular} \\ \hline
\begin{tabular}[c]{@{}l@{}}Xử lý trực tiếp \\ ảnh độ phân giải cao\end{tabular} &
  \begin{tabular}[c]{@{}l@{}}Tăng kích thước ảnh \\ hoặc số lượng patch\end{tabular} &
        \begin{tabular}[c]{@{}l@{}}Giữ lại nhiều chi tiết \\ hơn. FPT \cite{FPT} \\ xem xét sự đánh đổi \\ giữa khả năng bảo toàn \\ chi tiết và chi phí bộ \\ nhớ trong ảnh y khoa\end{tabular} &
  \begin{tabular}[c]{@{}l@{}}Số lượng đơn vị biểu \\ diễn và chi phí của cơ \\ chế chú ý tăng nhanh.\end{tabular} \\ \hline
\begin{tabular}[c]{@{}l@{}}AnyRes và \\ Higher-AnyRes\end{tabular} &
  \begin{tabular}[c]{@{}l@{}}Kết hợp ảnh toàn cục \\ với các vùng ảnh theo \\ lưới thích nghi và giới \\ hạn số đơn vị biểu diễn \\ bằng phép nội suy\end{tabular} &
  \begin{tabular}[c]{@{}l@{}}Tăng độ phủ không \\ gian và cải thiện các \\ tác vụ phụ thuộc vào \\ chi tiết \cite{LLaVANeXTAblations2024}\end{tabular} &
        \begin{tabular}[c]{@{}l@{}}Lưới đều có thể xử lý \\ nhiều vùng không liên \\ quan. Bằng chứng thực \\ nghiệm chủ yếu đến từ \\ dữ liệu phi y khoa.\end{tabular} \\ \hline
Latent resampling &
  \begin{tabular}[c]{@{}l@{}}Nén tập đơn vị biểu diễn \\ đầu vào thành một số \\ lượng latent cố định\end{tabular} &
  \begin{tabular}[c]{@{}l@{}}Kiểm soát độ dài \\ chuỗi và chi phí kết \\ nối đa phương thức\end{tabular} &
  \begin{tabular}[c]{@{}l@{}}Có nguy cơ làm mất \\ các tín hiệu nhỏ và phụ \\ thuộc vào khả năng tối \\ ưu các truy vấn học \\ được \cite{Jaegle2021Perceiver,Alayrac2022Flamingo}.\end{tabular} \\ \hline
\begin{tabular}[c]{@{}l@{}}Toàn cục-cục bộ \\ đa góc nhìn\end{tabular} &
  \begin{tabular}[c]{@{}l@{}}Kết hợp ảnh toàn cục \\ với nhiều vùng cục bộ \\ được trích xuất từ ảnh gốc\end{tabular} &
        \begin{tabular}[c]{@{}l@{}}Phù hợp với các dấu \\ hiệu bệnh lý nhỏ và \\ khu trú. MedBridge \cite{Li2026MedBridge} \\ cho thấy hiệu quả trên \\ ảnh X-quang ngực\end{tabular} &
  \begin{tabular}[c]{@{}l@{}}Làm tăng chi phí khi \\ sử dụng mô hình nền \\ tảng nhiều lần, hiệu \\ quả phụ thuộc vào \\ cách lựa chọn vùng \\ và chưa được kiểm \\ chứng trực tiếp trên \\ ảnh X-quang xương.\end{tabular} \\ \hline
\end{tabular}%
}
