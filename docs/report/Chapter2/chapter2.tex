...
\textbf{1} &
  \textbf{\begin{tabular}[c]{@{}l@{}}ConVIRT \cite{ConVIRT}\\ (2020)\end{tabular}} &
  \begin{tabular}[c]{@{}l@{}}Học tương\\ phản\end{tabular} &
  \multicolumn{1}{l|}{ResNet50} &
  \multicolumn{1}{l|}{BERT} &
  \multicolumn{1}{l|}{\begin{tabular}[c]{@{}l@{}}Căn chỉnh \\ tương phản\end{tabular}} &
  N/a \\ \hline
\textbf{2} &
  \textbf{\begin{tabular}[c]{@{}l@{}}GLoRIA \cite{GLoRIA}\\ (2021)\end{tabular}} &
  \begin{tabular}[c]{@{}l@{}}Học tương\\ phản\end{tabular} &
  \multicolumn{1}{l|}{ResNet50} &
  \multicolumn{1}{l|}{BioClinicalBERT} &
  \multicolumn{1}{l|}{\begin{tabular}[c]{@{}l@{}}Căn chỉnh tương phản \\ Toàn cục - \\ Cục bộ\end{tabular}} &
  N/a \\ \hline
\textbf{3} &
  \textbf{\begin{tabular}[c]{@{}l@{}}MedCLIP \cite{MedCLIP}\\ (2022)\end{tabular}} &
  \begin{tabular}[c]{@{}l@{}}Học tương\\ phản\end{tabular} &
  \multicolumn{1}{l|}{\begin{tabular}[c]{@{}l@{}}Swin Transformer, \\ ResNet50\end{tabular}} &
  \multicolumn{1}{l|}{BioClinicalBERT} &
  \multicolumn{1}{l|}{\begin{tabular}[c]{@{}l@{}}Căn chỉnh tăng cường \\ Tri thức\end{tabular}} &
  N/a \\ \hline
\textbf{4} &
  \textbf{\begin{tabular}[c]{@{}l@{}}BiomedCLIP \cite{BiomedCLIP}\\ (2023)\end{tabular}} &
  \begin{tabular}[c]{@{}l@{}}Học tương\\ phản\end{tabular} &
  \multicolumn{1}{l|}{ViT-Base} &
  \multicolumn{1}{l|}{PubMedBERT} &
  \multicolumn{1}{l|}{\begin{tabular}[c]{@{}l@{}}Căn chỉnh\\ tương phản
  \end{tabular}} &
  N/a \\ \hline
\end{tabular}
}
...
Ảnh đầu vào $x$ trước hết được tiền xử lý theo cấu hình của BiomedCLIP, bao gồm thay đổi kích thước, cắt vùng trung tâm và chuẩn hóa, để thu được tensor ảnh có kích thước $224\times224$. Ảnh sau đó được chia thành các mảnh ảnh có kích thước $16\times16$ và đưa qua bộ mã hóa thị giác ViT-B/16 để tạo biểu diễn ảnh.
...
Hướng tiếp cận này chèn thêm các tham số (vectơ) có thể học được vào không gian đầu vào của mô hình mà không thay đổi cấu trúc mạng bên trong \cite{Lester2021PromptTuning}.
\begin{itemize}
...
Phương pháp này chèn các mô-đun mạng nơ-ron siêu nhẹ, còn được gọi là bộ điều hợp vào giữa các lớp kiến trúc của mạng học sâu \cite{Houlsby2019Adapters}.
\begin{itemize}
...
Tinh chỉnh thích nghi hạng thấp LoRA \cite{Hu2022LoRA} giữ cố định trọng số nền và bổ sung các ma trận hạng thấp có thể huấn luyện để tham số hóa phần cập nhật trọng số. Cách tiếp cận này sử dụng một lượng nhỏ tham số huấn luyện nhưng đem lại hiệu quả gần như tương đương với tinh chỉnh toàn phần.
\begin{itemize}
    \item \textbf{Điểm mạnh:} Giảm đáng kể số tham số cần cập nhật và bộ nhớ tối ưu hóa với hiệu suất gần như tương đương với tinh chỉnh toàn phần. Ngoài ra, các ma trận cập nhật có thể được hợp nhất vào trọng số nền khi cách triển khai hỗ trợ.
...
\textbf{2} &
  \textbf{CLIPath (2023) \cite{CLIPath}} &
...
\caption{Tổng hợp các phương pháp tinh chỉnh hiệu quả tham số cho các mô hình ngôn ngữ trực quan y khoa nổi bật}
\label{tab:VLMTransferlearningMethodPropose}
\end{table}

\section{Các phương pháp phát hiện dữ liệu ngoài phân phối}

Phát hiện dữ liệu ngoài phân phối - OOD nhằm nhận diện các mẫu có đặc trưng khác biệt đáng kể so với phân phối dữ liệu mà mô hình đã quan sát trong quá trình huấn luyện. Trong phân tích ảnh y khoa, OOD có thể đóng vai trò như một tín hiệu cảnh báo bổ trợ khi mô hình tiếp nhận các trường hợp chưa quen thuộc hoặc khác biệt so với dữ liệu huấn luyện. Tuy nhiên, tín hiệu này không đồng nghĩa với việc bảo đảm an toàn cho hệ thống và cũng không thể thay thế quy trình đánh giá lâm sàng của bác sĩ. Xét theo nguyên lý và cách thức xác định mức độ bất thường của mẫu đầu vào, các phương pháp phát hiện OOD hiện nay có thể được phân thành một số nhóm chính như sau.
...
\begin{itemize}
    ...
    \begin{equation}
    d_M(z,\mu_k)=\sqrt{(z-\mu_k)^\top(\Sigma+\lambda_{\mathrm{cov}} I)^{-1}(z-\mu_k)}
    \label{eq:mahalanobis}
    \end{equation}
    Trong đó, $z$ là vectơ biểu diễn của mẫu kiểm thử, $\mu_k$ là trung bình đặc trưng của lớp $k$, $\Sigma$ là ma trận hiệp phương sai, $I$ là ma trận đơn vị và $\lambda_{\mathrm{cov}}$ là hệ số điều chuẩn hiệp phương sai. Ký hiệu $\lambda_{\mathrm{cov}}$ chỉ được dùng cho phép điều chuẩn này, khác với ngưỡng cảnh báo dữ liệu ngoài phân phối $\tau_{\mathrm{alert}}$ ở Chương~\ref{Chapter3}. Độ tin cậy của phép đo phụ thuộc vào chất lượng ước lượng hiệp phương sai và mức phù hợp của giả định phân phối.
    \item \textbf{Khoảng cách láng giềng gần nhất kNN \cite{Sun2022KNN}:}
    Thay vì mô hình hóa không gian đặc trưng bằng một dạng phân phối tham số, phương pháp kNN đánh giá mẫu kiểm thử dựa trên quan hệ lân cận trực tiếp với các biểu diễn của dữ liệu trong phân phối đã quan sát. Với tập tham chiếu $\mathcal{Z}_{\mathrm{ID}}=\{z_i\}_{i=1}^{N}$, tập $k$ láng giềng gần nhất của biểu diễn kiểm thử $z$ được xác định bởi:
    \begin{equation}
    \mathcal{N}_k(z)=\operatorname{kNN}(z,\mathcal{Z}_{\mathrm{ID}}).
    \label{eq:knn_neighbors}
    \end{equation}
...
\pagebreak
\begin{itemize}
    \item \textbf{Học tương phản:} Học biểu diễn bằng cách kéo gần các mẫu tương đồng và đẩy xa các mẫu khác lớp trong không gian đặc trưng \cite{Chen2020SimCLR,He2020MoCo}.
    \item \textbf{Mất mát tâm lớp \cite{CenterLoss}:} Giảm phương sai nội lớp bằng cách học một tâm biểu diễn cho mỗi lớp và ép các đặc trưng tiến gần tâm tương ứng.
...
Độ trung thực của lời giải thích thường được kiểm tra bằng can thiệp đầu vào với phép loại bỏ hoặc phép khôi phục \cite{Petsiuk2018RISE}. Trong phép loại bỏ, các thành phần được xếp hạng cao được thay dần bằng mốc tham chiếu. Khi đó, xác suất lớp đích giảm nhanh hơn so với thứ tự ngẫu nhiên cung cấp bằng chứng thuận lợi hơn cho thứ tự quy đóng. Phép khôi phục thực hiện theo chiều ngược lại. Kết quả phụ thuộc vào mốc thay thế, tỷ lệ can thiệp và không gian biểu diễn, do đó cần báo cáo giao thức cùng đối chứng ngẫu nhiên thay vì chỉ dựa vào bản đồ nhiệt.

Trọng số chú ý mô tả cách mô-đun phân bổ tương tác trong một lần truyền thuận nhưng không mặc nhiên là lời giải thích trung thực cho quyết định. Mối quan hệ giữa chú ý và giải thích vẫn phụ thuộc vào định nghĩa, mô hình và giao thức đánh giá cụ thể \cite{Jain2019AttentionNotExplanation,Wiegreffe2019AttentionNotNotExplanation}. Ngoài độ trung thực, đánh giá trong y khoa còn cần phân biệt độ ổn định của bản đồ, mức phù hợp với chú thích tổn thương và nhận định của chuyên gia. Khóa luận vì vậy sử dụng tích phân gradient và can thiệp đầu vào để khảo sát nguồn thông tin liên hệ với dự đoán, đồng thời không diễn giải bản đồ quy đóng như bằng chứng định vị hoặc xác nhận lâm sàng.
...
Mặc dù đạt được nhiều kết quả tích cực, các nghiên cứu hiện có vẫn chưa giải quyết đồng thời các yêu cầu của bài toán phân lớp ảnh X-quang xương đa phương thức, các vấn đề chính được tổng hợp như sau:

Thứ nhất, phần lớn mô hình y khoa đa phương thức được phát triển trên X-quang ngực hoặc các bộ dữ liệu lớn với X-quang chiếm phần không lớn. Ảnh X-quang xương có phạm vi giải phẫu, tư thế chụp và kiểu tổn thương đa dạng, trong khi số lượng mẫu giữa các lớp thường mất cân bằng đáng kể. Vì vậy, hiệu quả trên các miền dữ liệu khác chưa thể được chuyển trực tiếp sang bài toán này.
...
LoRA \cite{Hu2022LoRA} được sử dụng để thích nghi hai bộ mã hóa của BiomedCLIP với miền ảnh X-quang xương mà không cần cập nhật toàn bộ tham số nền. Lựa chọn này giảm số tham số cần cập nhật và hướng đến tiết kiệm bộ nhớ huấn luyện. Việc giữ cố định tham số nền cũng nhằm hạn chế thay đổi quá mức biểu diễn tiền huấn luyện khi một số lớp có ít mẫu; hiệu quả phân lớp và chi phí thực tế được đánh giá ở Chương~\ref{Chapter4}.

Bên cạnh đó, trong hàm mất mát tương phản InfoNCE nguyên bản \cite{Oord2018CPC} được BiomedCLIP sử dụng, các mẫu khác nhau trong cùng lô được xem là các cặp âm, kể cả khi chúng thuộc cùng một lớp hoặc có nội dung lâm sàng tương đồng, gây ra hiện tượng âm tính giả. Do đó, khóa luận sử dụng hàm mất mát khớp ngữ nghĩa kế thừa từ MedCLIP \cite{MedCLIP} và xây dựng phân phối đích mềm theo nhãn lớp. Mục tiêu này vẫn ưu tiên sự tương hợp của cặp ảnh-văn bản tương ứng, đồng thời giảm ảnh hưởng của các cặp âm tính giả giữa những mẫu có quan hệ ngữ nghĩa gần nhau.
...
Hai bộ mã hóa của BiomedCLIP tạo biểu diễn riêng cho ảnh và văn bản, chưa trực tiếp thực hiện dung hợp ở cấp độ cục bộ. Trong ứng dụng Hỏi-Đáp thị giác y tế (VQA), nhóm tác giả tích hợp BiomedCLIP vào khung METER để trao đổi thông tin giữa hai phương thức. Dựa vào đó, khóa luận đề xuất tích hợp một cơ chế dung hợp. Thay vì chỉ ghép nối hai biểu diễn toàn cục, khóa luận lựa chọn cơ chế chú ý chéo hai chiều, có nguyên lý trao đổi thông tin giữa hai luồng ảnh-văn bản tương tự ViLBERT \cite{Lu2019ViLBERT}, để đặc trưng ảnh lựa chọn các thông tin lâm sàng liên quan, đồng thời đặc trưng văn bản định hướng mô hình đến các vùng ảnh phù hợp. Thiết kế này cho phép mô hình hóa quan hệ cụ thể giữa dấu hiệu X-quang và ngữ cảnh lâm sàng.

\textbf{Về phân lớp đa lớp và mất cân bằng dữ liệu:}
Phương pháp sử dụng hàm mất mát entropy chéo có trọng số nhằm giảm ảnh hưởng của sự chênh lệch số lượng mẫu giữa các lớp.

\textbf{Về phát hiện dữ liệu ngoài phân phối:}
Ngoài nhiệm vụ chính, khóa luận bổ sung cơ chế phát hiện dữ liệu ngoài phân phối ở giai đoạn hậu xử lý mà không cần mẫu OOD khi huấn luyện. Phương pháp sử dụng kNN trực tiếp trên đặc trưng ảnh và văn bản từ hai bộ mã hóa BiomedCLIP, với hai ngân hàng tham chiếu $\mathcal{B}{I}$ và $\mathcal{B}{T}$ được xây dựng từ tập huấn luyện. Với mẫu mới, hai đặc trưng được đối sánh riêng với ngân hàng tương ứng để tạo tín hiệu khoảng cách lân cận và cảnh báo OOD theo ngưỡng hiệu chỉnh trên tập xác thực; chi tiết cách tính và kết hợp hai tín hiệu được trình bày trong Chương~\ref{Chapter3}.
