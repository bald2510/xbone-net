...
\centering
    ...
    \caption{Sơ đồ tích hợp hệ thống hỗ trợ chẩn đoán (Tham khảo từ quy trình thực tế tại Bệnh viện Nguyễn Tri Phương \cite{QuyTrinhBVNTP}).}
...
Đề tài hướng đến việc nghiên cứu và xây dựng một hệ thống học sâu đa phương thức phục vụ hỗ trợ phân loại bệnh lý trên ảnh X-quang xương, khai thác đồng thời dữ liệu hình ảnh và báo cáo lâm sàng nhằm nâng cao hiệu quả phân loại so với các phương pháp hiện có.
...
\begin{enumerate}
  ...
  \item Nghiên cứu chiến lược dung hợp đặc trưng đa phương thức, cụ thể là cơ chế chú ý chéo và so sánh với các phương pháp khác, nhằm xác định phương pháp kết hợp hiệu quả nhất giữa đặc trưng hình ảnh và văn bản.
  ...
  \item Nghiên cứu cơ chế phát hiện dữ liệu ngoài phân phối nhằm tăng khả năng nhận diện các mẫu bệnh lý chưa từng xuất hiện trong dữ liệu huấn luyện, nâng cao độ tin cậy của hệ thống trong môi trường lâm sàng thực tế.
...
\begin{itemize}
  ...
  \item Các chiến lược dung hợp đặc trưng đa phương thức, bao gồm cơ chế chú ý chéo và cộng đặc trưng, nhằm kết hợp thông tin từ ảnh X-quang và báo cáo lâm sàng.
  \item Các phương pháp tinh chỉnh tham số hiệu quả cho các mô hình tiền huấn luyện quy mô lớn nhằm giảm chi phí tính toán.
  \item Các kiến trúc bộ phân loại, phục vụ bài toán phân loại đa lớp trên dữ liệu mất cân bằng.
\end{itemize}
...
\begin{itemize}
  \item \textbf{Dữ liệu:} Nghiên cứu sử dụng bộ dữ liệu BTXRD gồm 10 lớp bệnh lý xương, và dữ liệu được thu thập từ Bệnh viện Chấn thương Chỉnh hình Thành phố Hồ Chí Minh. Dữ liệu có đặc điểm phân bố mất cân bằng giữa các lớp bệnh.
  \item \textbf{Dữ liệu văn bản:} Trong quá trình đánh giá, hệ thống sử dụng báo cáo lâm sàng (chứa thông tin triệu chứng, tiền sử bệnh) thay vì báo cáo đọc ảnh X-quang, nhằm phù hợp với quy trình chẩn đoán thực tế tại các cơ sở y tế.
...
\begin{itemize}
    \item \textbf{Câu hỏi nghiên cứu 1:} Việc bổ sung thông tin từ báo cáo lâm sàng vào mô hình dựa trên ảnh X-quang có giúp cải thiện đáng kể hiệu quả phân loại bệnh lý xương không?
    
    \textbf{Giả thuyết 1:} Mô hình đa phương thức đạt kết quả cao hơn đáng kể so với mô hình chỉ sử dụng ảnh.

    \item \textbf{Câu hỏi nghiên cứu 2:} Cơ chế dung hợp chú ý chéo có hiệu quả hơn phương pháp cộng đặc trưng truyền thống trong việc kết hợp đặc trưng ảnh và văn bản không?

    \textbf{Giả thuyết 2:} Cơ chế dung hợp chú ý chéo đạt kết quả cao hơn phương pháp cộng đặc trưng nhờ khả năng chú ý chọn lọc vào các vùng thông tin liên quan.

    \item \textbf{Câu hỏi nghiên cứu 3:} Chiến lược tinh chỉnh hiệu quả tham số có thể đạt hiệu quả tương đương hoặc vượt trội so với tinh chỉnh toàn bộ tham số trong khi chỉ cập nhật một phần nhỏ tham số không?
    
    \textbf{Giả thuyết 3:} Chiến lược tinh chỉnh hiệu quả tham số đạt kết quả cao hơn tinh chỉnh toàn bộ tham số nhờ tránh quá khớp trên dataset nhỏ.

    \item \textbf{Câu hỏi nghiên cứu 4:} Hệ thống đề xuất (XBone-Net) có vượt trội so với các mô hình nền tảng được tinh chỉnh truyền thống trên bài toán phân loại bệnh lý xương không?
     
    \textbf{Giả thuyết 4:} XBone-Net đạt kết quả cao hơn trong tất cả các mô hình được so sánh nhờ kiến trúc đề xuất.
\end{itemize}
...
\begin{enumerate}
    \item \textbf{Kiến trúc XBone-Net:} Đề xuất kiến trúc học sâu đa phương thức kết hợp cơ chế dung hợp chú ý chéo và bộ phân loại dựa trên nguyên mẫu kế thừa từ nền tảng mô hình ngôn ngữ trực quan BiomedCLIP, được tinh chỉnh hiệu quả bằng chiến lược tinh chỉnh hiệu quả tham số cho bài toán phân loại bệnh lý xương.

    \item \textbf{Quy trình huấn luyện hai giai đoạn:} Thiết kế cơ chế huấn luyện gồm Giai đoạn 1 nhằm căn chỉnh biểu diễn đa phương thức và Giai đoạn 2 nhằm phân loại bệnh lý. Quy trình hai giai đoạn giúp mô hình học được mối quan hệ ngữ nghĩa giữa ảnh X-quang và báo cáo lâm sàng trước khi thực hiện phân loại.

    \pagebreak

    \item \textbf{Khai thác báo cáo lâm sàng:} Chứng minh hiệu quả của việc sử dụng báo cáo lâm sàng (chứa thông tin triệu chứng, tiền sử bệnh) thay vì báo cáo đọc ảnh X-quang trong quá trình đánh giá, phù hợp hơn với quy trình chẩn đoán thực tế tại các cơ sở y tế.

    \item \textbf{Tinh chỉnh hiệu quả tham số:} Chứng minh rằng phương pháp tinh chỉnh hiệu quả tham số có thể đạt hiệu quả phân loại vượt trội so với tinh chỉnh toàn bộ tham số trong khi chỉ cập nhật một phần nhỏ tổng số tham số, đặc biệt phù hợp với bộ dữ liệu y tế có quy mô nhỏ và phân bố mất cân bằng.

    \item \textbf{Phát hiện dữ liệu ngoài phân phối:} Tích hợp cơ chế phát hiện mẫu ngoài phân phối dựa trên khoảng cách Mahalanobis với các nguyên mẫu đã được học trong không gian đặc trưng, giúp hệ thống nhận diện và cảnh báo khi gặp các loại bệnh lý chưa từng xuất hiện trong dữ liệu huấn luyện.

    \item \textbf{Khả năng giải thích:} Xây dựng công cụ trực quan hóa bản đồ chú ý từ cơ chế dung hợp chú ý chéo, cho phép bác sĩ kiểm chứng cơ sở đưa ra quyết định của mô hình.
\end{enumerate}
