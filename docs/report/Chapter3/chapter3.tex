...
\begin{enumerate}
    \item \textbf{Tiền xử lý ảnh độ phân giải cao:} ảnh X-quang được phủ kín bằng lưới lát $224\times224$ với stride 224. Chỉ các vùng đệm máy chụp gần như đồng nhất mới bị loại bỏ; pipeline không sử dụng nhãn, hộp tổn thương hay mô hình đề xuất vùng. Khi số lát vượt quá 64, ảnh được thu nhỏ đẳng hướng vừa đủ để toàn bộ trường nhìn vẫn được bao phủ.
    \item \textbf{Mã hóa đa tỉ lệ:} BiomedCLIP trích xuất một đặc trưng ảnh toàn cục và các patch token cục bộ. Mỗi lát được gộp thành lưới $2\times2$ token; bộ resampler có truy vấn học được và thông tin tọa độ nén toàn bộ token hợp lệ thành 24 visual token. Trong khi huấn luyện, gradient từ nhánh local được truyền vào visual LoRA; activation của từng nhóm tile được tính lại khi backward bằng gradient checkpointing để giới hạn bộ nhớ.
