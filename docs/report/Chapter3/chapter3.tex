...
Trong đó, $x_i^{\mathrm{img}}\in\mathbb{R}^{H_i\times W_i\times C_{\mathrm{in}}}$ là ảnh X-quang của mẫu thứ $i$; $H_i$, $W_i$ và $C_{\mathrm{in}}$ lần lượt là chiều cao, chiều rộng và số kênh ảnh. Thông tin lâm sàng được ký hiệu $x_i^{\mathrm{txt}}=\left(w_{i,1},\ldots,w_{i,L_i}\right)$, với $L_i$ là số token trước khi cắt hoặc đệm. Nhãn $y_i$ thuộc tập chỉ số lớp $\mathcal{Y}=\{1,\ldots,K\}$, còn $N$ là số mẫu huấn luyện.
...
Hai đại lượng trên được chuẩn hóa min--max trong phạm vi các cửa sổ của cùng ảnh; nếu mọi giá trị bằng nhau thì kết quả chuẩn hóa được đặt bằng $0$. Điểm tổng hợp dành trọng số $0{,}55$ cho năng lượng Laplacian, $0{,}35$ cho entropy và $0{,}10$ cho tỷ lệ tiền cảnh. Đây là quy tắc lựa chọn của hệ thống, không phải công thức được kế thừa từ các công trình được trích dẫn.

Bốn cửa sổ được chia thành hai vai trò. Hai cửa sổ bao phủ được chọn gần các vị trí phân bố trên trục dài của vùng nội dung và ưu tiên vùng chứa nhiều tiền cảnh. Hai cửa sổ tiêu điểm ưu tiên điểm tổng hợp cao, đồng thời được khuyến khích cách xa các vùng đã chọn với trọng số đa dạng $0{,}35$. Các ứng viên có mức giao trên hợp lớn hơn $0{,}30$ bị loại. Nếu số ứng viên hợp lệ không đủ, quy trình lần lượt mở rộng sang toàn bộ lưới, bỏ ràng buộc chồng lấp và lặp vùng theo thứ tự xác định để luôn tạo bốn đầu vào cục bộ.
...
Gọi $\boldsymbol{\ell}_{i,n}$ và $\boldsymbol{b}_{i,n}\in[0,1]^4$ lần lượt là đơn vị cục bộ thứ $n$ và hộp tọa độ tương ứng. Tọa độ được chiếu qua một mạng hai lớp, cộng có kiểm soát vào nội dung và chuẩn hóa lại, tạo ra token ảnh có thông tin vị trí:
...
Bổ sung cho các bằng chứng hình thái được trích xuất từ ảnh X-quang, nhánh biểu diễn văn bản có nhiệm vụ mã hóa thông tin lâm sàng $x_i^{\mathrm{txt}}$ thành các đặc trưng ngữ cảnh theo từng token và một vectơ đại diện cho toàn bộ chuỗi. Trong khóa luận, nhánh này được kế thừa từ bộ mã hóa văn bản của BiomedCLIP \cite{BiomedCLIP}, với thành phần chính là PubMedBERT. Khác với các mô hình ngôn ngữ được tiền huấn luyện trên văn bản tổng quát, PubMedBERT được tiền huấn luyện từ đầu trên kho ngữ liệu y sinh và sử dụng bộ từ vựng được xây dựng trực tiếp từ miền dữ liệu này. Nhờ đó, mô hình phù hợp hơn với các thuật ngữ chuyên ngành xuất hiện trong thông tin lâm sàng.

\subsection{Tiền xử lý và mã hóa đầu vào}

Trước khi đưa vào PubMedBERT, chuỗi văn bản $x_i^{\mathrm{txt}}$ được phân tách thành các đơn vị từ con bằng bộ mã hóa WordPiece tương ứng với mô hình. Hai token đặc biệt \texttt{[CLS]} và \texttt{[SEP]} lần lượt được thêm vào đầu và cuối chuỗi. Chuỗi sau khi mã hóa được cắt hoặc đệm đến độ dài tối đa $T_T$:
...
Mô hình kế thừa bộ mã hóa văn bản PubMedBERT của BioMedCLIP \cite{BiomedCLIP}. Chuỗi chỉ số token $\boldsymbol{u}_i$ cùng mặt nạ chú ý $\boldsymbol{m}_i$ được đưa qua bộ mã hóa và lớp chiếu để tạo chuỗi đặc trưng có cùng chiều $D=512$ với nhánh ảnh:
...
$\mathcal{F}_{T}$ gồm bộ mã hóa văn bản và lớp chiếu của BioMedCLIP, còn $T_T$ là độ dài chuỗi sau khi cắt hoặc đệm. Mặt nạ $\boldsymbol{m}_i$ loại bỏ ảnh hưởng của các vị trí đệm. Chỉ số $0$ biểu thị token \texttt{[CLS]}, được dùng để tạo vectơ văn bản toàn cục đã chuẩn hóa:
...
Trong đó, $d_h=D/H_a$ là số chiều của một trong $H_a$ đầu chú ý. Mỗi đầu trong mỗi hướng có các ma trận $\boldsymbol{W}_{Q}^{(h)},\boldsymbol{W}_{K}^{(h)},\boldsymbol{W}_{V}^{(h)}$ riêng, còn $\boldsymbol{W}_{O}$ chiếu kết quả sau khi ghép các đầu. Mặt nạ $\boldsymbol{M}$ nhận giá trị $-\infty$ tại token đệm và $0$ tại token hợp lệ.
...
Hình \ref{fig:ch3_fusion} cho thấy hai hướng chú ý được thực hiện song song. Hai mô-đun không chia sẻ tham số vì vai trò truy vấn-ngữ cảnh đảo ngược nhau. Kết nối phần dư giữ lại token toàn cục, còn MLP học cách tổng hợp hai biểu diễn đã được bổ sung ngữ cảnh.
...
Điểm tương đồng giữa mẫu và tâm lớp được xác định bằng côsin:
...
Đầu phân loại chuyển điểm tương đồng thành logit sử dụng hệ số tỷ lệ $\alpha_c$ và một độ lệch học được riêng cho từng lớp:
...
trong đó $\alpha_c$ điều chỉnh độ sắc của phân phối xác suất, còn $b_k$ được tối ưu trong pha 2. Khi các độ lệch bằng nhau, nhãn dự đoán tương đương với tâm có côsin lớn nhất; khi $b_k$ được học, ranh giới còn được hiệu chỉnh theo lớp. Các tâm $\boldsymbol{c}_k$ là thống kê của tập huấn luyện, không phải tham số nhận đạo hàm.
...
Trong đó, $\mathcal{P}_{i}$ là tập chỉ số token cục bộ hợp lệ, còn $\alpha_{i,n}$ biểu thị mức đóng góp tương đối của token cục bộ thứ $n$ vào biểu diễn của ảnh $i$. Trọng số này không phải là một tham số được học trực tiếp mà được tính riêng cho từng ảnh bằng Softmax trên các điểm số sinh từ token cục bộ $\widetilde{\boldsymbol{\ell}}_{i,n}$ và đặc trưng toàn cục $\boldsymbol{g}_{i}$ thông qua một hàm chấm điểm có tham số học được. Do đó, $\alpha_{i,n}\geq0$, $\sum_{n\in\mathcal{P}_{i}}\alpha_{i,n}=1$; giá trị càng lớn thì token tương ứng đóng góp càng nhiều. Vectơ $\boldsymbol{v}_{i}^{\mathrm{local}}$ tóm tắt thông tin cục bộ, còn $\overline{\boldsymbol{v}}_{i}$ giữ đặc trưng toàn cục làm thành phần chính và bổ sung $0{,}25$ lần bản tóm tắt này.
...
Sau giai đoạn này, hai bộ mã hóa đã được điều chỉnh để tạo ra không gian ảnh-văn bản phù hợp hơn với dữ liệu X-quang xương. Tuy nhiên, các đặc trưng của cặp ảnh-văn bản mới chỉ được căn chỉnh gần nhau hơn ở mức biểu diễn trong không gian nhúng và chưa được tối ưu trực tiếp cho quyết định phân loại đa lớp. Vì vậy, giai đoạn thứ hai tiếp tục sử dụng các bộ mã hóa đã thích nghi, đồng thời đưa mô-đun dung hợp và mạng nguyên mẫu vào quá trình huấn luyện có giám sát.
...
Các tâm lớp được khởi tạo từ toàn bộ tập huấn luyện trước epoch đầu tiên. Trong mỗi epoch, chúng được giữ cố định như buffer và không nhận gradient. Ngay sau epoch, mô hình chuyển sang chế độ đánh giá, tính lại embedding của tập huấn luyện bằng trạng thái tham số mới nhất, cập nhật các tâm theo Công thức \ref{eq:ch3_prototype}, rồi mới đánh giá trên tập kiểm định. Trạng thái tốt nhất được lựa chọn theo độ đo đã định trước; cuối cùng, các tâm được tính lại từ tập huấn luyện bằng trạng thái này. Tập kiểm thử không tham gia cập nhật tham số, ước lượng tâm hoặc lựa chọn mô hình.
...
Điểm $s_{\mathrm{OOD}}(\boldsymbol{z})$ là khoảng cách nhỏ nhất từ biểu diễn của mẫu mới đến các lớp đã biết. Giá trị càng lớn cho thấy mẫu càng xa toàn bộ phân bố lớp ID. Ngưỡng $\delta_{\mathrm{OOD}}$ được hiệu chỉnh chỉ bằng dữ liệu ID huấn luyện và kiểm định. Quy tắc đầu ra là:
...
\begin{cases}
\mathrm{OOD},
& \text{nếu }s_{\mathrm{OOD}}(\boldsymbol{z})>\delta_{\mathrm{OOD}},\\[4pt]
\displaystyle
...
Chương này trình bày kiến trúc, quá trình huấn luyện và cơ chế suy luận của XBone-Net cho bài toán phân loại ảnh X-quang xương kết hợp thông tin lâm sàng. Các nội dung chính gồm:
...
\begin{enumerate}
    ...
    \item \textbf{Huấn luyện hai giai đoạn và phân loại nguyên mẫu:} Giai đoạn đầu khớp ngữ nghĩa ảnh-văn bản bằng đích mềm; giai đoạn sau học dung hợp và phân loại theo nguyên mẫu với mất mát có trọng số lớp.
