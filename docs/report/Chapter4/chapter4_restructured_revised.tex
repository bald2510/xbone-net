...
Các trung bình trong hai bảng chỉ dùng để mô tả. Macro-F1 là độ đo chính và forest plot tương ứng được trình bày tại Hình~\ref{fig:full_shot_f1_forest}; các forest plot bổ sung cho Accuracy, Balanced Accuracy, Macro-AUROC và Macro-AUPRC được chuyển xuống Phụ lục, Hình~\ref{fig:appendix-full-shot-accuracy-forest}--\ref{fig:appendix-full-shot-auprc-forest}. Trong các biểu đồ, $\Delta$ được tính bằng XBone-Net trừ mô hình cơ sở. Khoảng tin cậy 95\% được ước lượng bằng bootstrap phân tầng theo bệnh nhân và hạt giống, còn p-value thô được tính bằng kiểm định hoán vị ghép cặp. Một khác biệt chỉ được xem là nhất quán khi khoảng tin cậy không chứa 0 và đồng thời $p<0{,}05$; các p-value không được hiệu chỉnh cho nhiều phép so sánh.

Theo tiêu chí trên, điểm Macro-F1 của XBone-Net cạnh tranh với toàn bộ các Med-VLM trên cả BTXRD và CTCH: không phép so sánh nào đồng thời có khoảng tin cậy loại trừ 0 và $p<0{,}05$. XBone-Net cao hơn rõ ràng so với ResNet-50 và DenseNet-121 trên phần lớn độ đo. Trong nhóm Med-VLM, XBone-Net cao hơn PubMedCLIP về Accuracy và cao hơn MedCLIP về Macro-AUROC trên BTXRD; LoRA-BiomedCLIP cao hơn XBone-Net về Balanced Accuracy trên BTXRD. Trên CTCH, XBone-Net cao hơn PubMedCLIP và MedCLIP về Macro-AUROC. Các so sánh còn lại chưa cung cấp bằng chứng khác biệt nhất quán, vì vậy được diễn giải là hiệu năng cạnh tranh thay vì thấp hơn hoặc cao hơn dựa riêng trên trung bình.
...
\begin{figure}[H]
\centering
\includegraphics[width=1\linewidth]{images/chapter4/full_shot_forest_f1_macro.png}
\caption{Khoảng tin cậy 95\% và p-value thô của chênh lệch Macro-F1 giữa XBone-Net và các mô hình cơ sở trong thiết lập toàn bộ dữ liệu}
\label{fig:full_shot_f1_forest}
\end{figure}
...
Bảng~\ref{tab:classification_few_shot} và Bảng~\ref{tab:classification_few_shot_ranking} mô tả hiệu năng theo ngân sách mẫu. Forest plot của độ đo chính Macro-F1 được trình bày tại Hình~\ref{fig:few_shot_f1_forest}; bốn độ đo bổ sung được báo cáo ở Phụ lục, Hình~\ref{fig:appendix-few-shot-accuracy-forest}--\ref{fig:appendix-few-shot-auprc-forest}. Các biểu đồ sử dụng khoảng Student-t 95\% và p-value thô của kiểm định t ghép cặp trên ba hạt giống chung, với $\Delta$ bằng XBone-Net trừ mô hình cơ sở. Do cỡ mẫu chỉ gồm ba cặp hạt giống, các kết quả suy luận này được xem là bằng chứng thăm dò.

\input{tables/chapter4/table_few_shot_classification}

\input{tables/chapter4/table_few_shot_ranking}

Trên cả BTXRD và CTCH, điểm Macro-F1 của XBone-Net cạnh tranh với LoRA-BiomedCLIP và LoRA-PubMedCLIP ở cả ba ngân sách vì mọi khoảng tin cậy đều chứa 0 và mọi p-value đều lớn hơn $0{,}05$. Trên CTCH với một mẫu mỗi lớp, XBone-Net cao hơn LoRA-BiomedCLIP về Accuracy, Macro-AUROC và Macro-AUPRC, đồng thời cao hơn LoRA-PubMedCLIP về hai độ đo xếp hạng. Ở 10 mẫu trên CTCH, LoRA-BiomedCLIP cao hơn XBone-Net về Macro-AUPRC; ở 20 mẫu, LoRA-PubMedCLIP cao hơn XBone-Net về Accuracy. Các phép so sánh còn lại chưa cho thấy khác biệt nhất quán.

Khi tăng từ một lên 10 mẫu, điểm F1 trung bình quan sát được của XBone-Net tăng $0{,}2682$ điểm trên BTXRD và $0{,}2093$ điểm trên CTCH. Mức tăng từ 10 lên 20 mẫu giảm còn $0{,}0900$ và $0{,}0355$ điểm. Xu hướng này chỉ mô tả đường học của XBone-Net và không thay thế kiểm định giữa các mô hình; ba mức ngân sách cũng chưa đủ để mô tả đầy đủ lợi ích biên của dữ liệu bổ sung.

\begin{figure}[H]
\centering
\includegraphics[width=1\linewidth]{images/chapter4/few_shot_forest_f1_macro.png}
\caption{Khoảng tin cậy 95\% và p-value thô của chênh lệch Macro-F1 trong thiết lập ít mẫu}
\label{fig:few_shot_f1_forest}
\end{figure}

\subsection{Hiệu quả tham số và chi phí suy luận}
\label{subsec:rq3_adaptation_results}
Bảng~\ref{tab:rq3_adaptation} đối chiếu ba mức thích nghi bộ mã hóa nền tảng trên CTCH. XBone-Net sử dụng thích nghi hạng thấp và cập nhật $8{,}63$ triệu tham số, tương đương $4{,}22\%$ tổng số tham số, trong khi tinh chỉnh toàn bộ cập nhật $199{,}43$ triệu tham số. Các chênh lệch hiệu năng trong bảng chỉ là mô tả trung bình vì phép đối chứng này chưa có khoảng tin cậy ghép cặp và p-value tương ứng. Do đó, kết luận được giới hạn ở việc XBone-Net duy trì hiệu năng quan sát được cạnh tranh trong khi cập nhật ít tham số hơn, không khẳng định thích nghi hạng thấp vượt trội về hiệu năng dự đoán.
...
\subsection{Tổng kết}
Trong thiết lập toàn bộ dữ liệu, XBone-Net có Macro-F1 cạnh tranh với các Med-VLM trên cả hai bộ dữ liệu và có lợi thế thống kê ở một số phép so sánh Accuracy hoặc Macro-AUROC, nhưng không vượt trội nhất quán trên mọi độ đo. Trong thiết lập ít mẫu, Macro-F1 tiếp tục cạnh tranh ở toàn bộ ngân sách; lợi thế và bất lợi có ý nghĩa chỉ xuất hiện ở một số độ đo trên CTCH. LoRA làm giảm mạnh số tham số cần cập nhật. Đổi lại, nhánh ảnh toàn cục--cục bộ và quy trình hai pha khiến mức tiết kiệm tham số không đồng nghĩa với giảm chi phí suy luận.
...
Bảng~\ref{tab:ablation_leave_one_out_classification} trình bày trung bình và độ lệch chuẩn của từng cấu hình. Forest plot của độ đo chính Macro-F1 được giữ tại Hình~\ref{fig:ablation_f1_forest}; các forest plot bổ sung được trình bày ở Phụ lục, Hình~\ref{fig:appendix-ablation-accuracy-forest}--\ref{fig:appendix-ablation-auprc-forest}. Trong các biểu đồ, $\Delta$ bằng XBone-Net trừ biến thể. Khoảng tin cậy 95\% được ước lượng bằng bootstrap phân tầng theo bệnh nhân và hạt giống, p-value thô được tính bằng kiểm định hoán vị ghép cặp và không được hiệu chỉnh cho nhiều phép so sánh.

Theo đồng thời khoảng tin cậy và p-value, XBone-Net cạnh tranh với gộp trung bình và nối đặc trưng trên cả năm độ đo. Cấu hình chỉ pha 2 và đầu tuyến tính có Accuracy cao hơn XBone-Net; ngược lại, XBone-Net có Macro-AUROC cao hơn cấu hình không dùng ảnh độ phân giải cao và đầu tuyến tính. Không phép so sánh nào cho thấy khác biệt nhất quán về Balanced Accuracy, Macro-F1 hoặc Macro-AUPRC. Vì vậy, các chênh lệch trung bình còn lại trong bảng không được diễn giải thành ưu thế hoặc bất lợi.

\input{tables/chapter4/table_ablation_leave_one_out_classification}

Đối với cấu hình không dùng ảnh độ phân giải cao, Macro-AUROC giảm một cách nhất quán so với XBone-Net, hỗ trợ vai trò của nhánh ảnh cục bộ đối với chất lượng xếp hạng. Tuy nhiên, khác biệt Accuracy có $p<0{,}05$ nhưng khoảng tin cậy vẫn chứa 0 nên không được xem là kết luận nhất quán. Với cấu hình chỉ pha 2, bằng chứng rõ chỉ xuất hiện ở Accuracy theo hướng có lợi cho biến thể; các độ đo còn lại chưa khác biệt rõ. Những kết quả này cho thấy đóng góp của từng thành phần phụ thuộc vào mục tiêu đánh giá thay vì tạo ra cải thiện đồng thời trên mọi độ đo.

\begin{figure}[H]
\centering
\includegraphics[width=1\linewidth]{images/chapter4/ablation_forest_f1_macro.png}
\caption{Khoảng tin cậy 95\% và p-value thô của chênh lệch Macro-F1 giữa XBone-Net và các biến thể ablation trên CTCH}
...
Trong nhóm mã hóa ảnh, các biến thể có trung bình quan sát được khác nhau theo từng độ đo, nhưng không được xem là cao hơn hoặc thấp hơn XBone-Net nếu chưa có khoảng tin cậy và p-value ghép cặp tương ứng. Riêng gộp trung bình đã được kiểm định trong nhóm leave-one-out và cạnh tranh với XBone-Net trên cả năm độ đo. Vì vậy, bảng bổ sung được dùng để nhận diện các hướng đánh đổi cần kiểm chứng tiếp, không dùng để xếp hạng chắc chắn các cơ chế mã hóa ảnh.

Trong nhóm huấn luyện, chỉ pha 2 chỉ có bằng chứng nhất quán về Accuracy cao hơn XBone-Net; khác biệt Macro-F1, Macro-AUROC và Macro-AUPRC chưa rõ. Trong nhóm dung hợp, phép nối đặc trưng cạnh tranh với XBone-Net trên toàn bộ năm độ đo được kiểm định. Trong nhóm đầu phân loại, đầu tuyến tính có Accuracy cao hơn nhưng Macro-AUROC thấp hơn XBone-Net, còn Macro-F1 chưa khác biệt rõ. Các cấu hình bổ sung chưa có kiểm định ghép cặp chỉ được mô tả bằng trung bình và không được dùng để khẳng định hơn hoặc kém.

\subsection{Tổng kết}
Không biến thể ablation nào cao hơn XBone-Net một cách nhất quán trên toàn bộ năm độ đo. XBone-Net cạnh tranh với gộp trung bình và nối đặc trưng; nhánh ảnh cục bộ và đầu phân lớp theo tâm lớp có bằng chứng thuận lợi về Macro-AUROC, trong khi chỉ pha 2 và đầu tuyến tính thuận lợi hơn về Accuracy. Cấu hình đầy đủ vì vậy được xem là một điểm đánh đổi giữa quyết định lớp và chất lượng xếp hạng, thay vì cấu hình vượt trội tuyệt đối hoặc thấp hơn các biến thể dựa trên trung bình.
...
Bảng~\ref{tab:rq4_ood_baselines} giữ cố định điểm Mahalanobis để so sánh ảnh hưởng của không gian biểu diễn. Các thứ hạng trong bảng chỉ là mô tả trung bình vì chưa có khoảng tin cậy ghép cặp và p-value cho phép so sánh này. BiomedCLIP không mẫu có AUROC-OOD quan sát được $0{,}6362$ trên nhóm ngoài không gian nhãn trong một lần đánh giá, còn XBone-Net có AUPR-Out quan sát được cao nhất trong nhóm các biểu diễn được huấn luyện trên kịch bản này. Trên BTXRD, phép nối đặc trưng có trung bình thuận lợi nhất ở cả ba độ đo. Do đó, kết quả chỉ cho thấy hiệu năng OOD phụ thuộc vào không gian biểu diễn, không khẳng định một cấu hình vượt trội thống kê trên mọi dạng dịch chuyển.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\linewidth]{images/chapter4/multimodal_ig_2842.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.85\linewidth]{images/chapter4/multimodal_ig_3021.png}
