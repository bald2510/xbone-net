...
BTXRD và CTCH được thiết lập dưới dạng bài toán phân loại đa lớp đơn nhãn, với $C=10$ đối với BTXRD và $C=22$ đối với CTCH. Đối với một tập đánh giá gồm $N$ mẫu, $y_i$ và $\widehat{y}_i$ lần lượt ký hiệu nhãn thật và nhãn dự đoán của mẫu thứ $i$, còn $p_{ic}$ là xác suất mô hình gán cho lớp $c$. Khi xem lớp $c$ là lớp dương và các lớp còn lại là lớp âm, $TP_c$, $FP_c$ và $FN_c$ lần lượt là số dương tính thật, dương tính giả và âm tính giả của lớp đó. Các độ đo được chia thành ba nhóm nhằm tách biệt hiệu năng phân loại, khả năng nhận diện dữ liệu ngoài phân phối và đặc tính của cơ chế chú ý--dung hợp.
...
\begin{equation}\operatorname{Accuracy}=\frac{1}{N}\sum_{i=1}^{N}\mathbb{I}\left(y_i=\widehat{y}_i\right).\label{eq:ch4_accuracy}\end{equation}

Trong Công thức \ref{eq:ch4_accuracy}, $N$ là số mẫu đánh giá, $y_i$ là nhãn thật, $\widehat{y}_i$ là nhãn dự đoán và $\mathbb{I}(\cdot)$ là hàm chỉ thị nhận giá trị một khi điều kiện đúng và bằng không trong trường hợp còn lại.
...
Trong Công thức \ref{eq:ch4_balanced_accuracy}, $C$ là tổng số lớp, $TP_c$ là số mẫu lớp $c$ được dự đoán đúng và $FN_c$ là số mẫu thuộc lớp $c$ nhưng bị dự đoán thành lớp khác. Tỷ số $TP_c/(TP_c+FN_c)$ là recall của lớp $c$.
...
Trong Công thức \ref{eq:ch4_macro_f1}, $C$ là tổng số lớp, $TP_c$ là số dự đoán đúng lớp $c$, $FP_c$ là số mẫu lớp khác bị dự đoán nhầm thành lớp $c$ và $FN_c$ là số mẫu lớp $c$ bị dự đoán thành lớp khác. Thành phần của lớp có mẫu số bằng không được quy ước bằng không.
...
Macro-AUROC đánh giá khả năng xếp hạng xác suất của từng lớp theo chiến lược one-versus-rest trên toàn bộ ngưỡng, sau đó trao trọng số bằng nhau cho các lớp hợp lệ.

\begin{equation}\operatorname{Macro\text{-}AUROC}=\frac{1}{\left|\mathcal{C}_{v}\right|}\sum_{c\in\mathcal{C}_{v}}\int_{0}^{1}\operatorname{TPR}_{c}(u)\,\mathrm{d}u.\label{eq:ch4_macro_auroc}\end{equation}

Trong Công thức \ref{eq:ch4_macro_auroc}, $\mathcal{C}_{v}$ là tập lớp có cả mẫu dương và mẫu âm, $|\mathcal{C}_{v}|$ là số lớp hợp lệ, $u\in[0,1]$ là một mức false-positive rate và $\operatorname{TPR}_{c}(u)$ là true-positive rate của lớp $c$ tại mức $u$. Tích phân là diện tích dưới đường ROC của lớp $c$.
...
\begin{equation}\operatorname{AP}_{c}=\sum_{k=1}^{K_c}\left(R_{c,k}-R_{c,k-1}\right)P_{c,k}.\label{eq:ch4_ap_class}\end{equation}

Trong Công thức \ref{eq:ch4_ap_class}, $\operatorname{AP}_{c}$ là average precision của lớp $c$, $K_c$ là số điểm ngưỡng trên đường precision--recall, còn $P_{c,k}$ và $R_{c,k}$ lần lượt là precision và recall tại điểm thứ $k$; $R_{c,0}=0$ là mức recall khởi đầu.

\begin{equation}\operatorname{Macro\text{-}AUPRC}=\frac{1}{\left|\mathcal{C}_{v}\right|}\sum_{c\in\mathcal{C}_{v}}\operatorname{AP}_{c}.\label{eq:ch4_macro_auprc}\end{equation}

Trong Công thức \ref{eq:ch4_macro_auprc}, $\mathcal{C}_{v}$ là tập lớp có cả mẫu dương và mẫu âm, $|\mathcal{C}_{v}|$ là số lớp hợp lệ và $\operatorname{AP}_{c}$ là đại lượng được xác định trong Công thức \ref{eq:ch4_ap_class}.

Macro-AUPRC được sử dụng vì nhạy với chất lượng nhận diện lớp dương khi số mẫu dương ít hơn đáng kể số mẫu âm. Độ đo này giúp phát hiện trường hợp AUROC tương đối cao nhưng precision của lớp hiếm vẫn thấp. Giá trị cao hơn biểu thị sự đánh đổi precision--recall tốt hơn; khi diễn giải cần đối chiếu với tỷ lệ lưu hành của từng lớp.
...
Trong Công thức \ref{eq:ch4_ece}, $M$ là số khoảng độ tin cậy, $B_m$ là tập mẫu thuộc khoảng thứ $m$, $|B_m|$ là số mẫu trong khoảng, $N$ là tổng số mẫu, $\operatorname{Acc}(B_m)$ là tỷ lệ dự đoán đúng và $\operatorname{Conf}(B_m)$ là trung bình xác suất lớn nhất trong khoảng. Báo cáo sử dụng $M=15$ khoảng có độ rộng bằng nhau.
...
\begin{equation}\boldsymbol{M}_{ab}=\sum_{i=1}^{N}\mathbb{I}\left(y_i=a\right)\mathbb{I}\left(\widehat{y}_i=b\right).\label{eq:ch4_confusion_matrix}\end{equation}

Trong Công thức \ref{eq:ch4_confusion_matrix}, $\boldsymbol{M}\in\mathbb{N}^{C\times C}$ là ma trận nhầm lẫn, $\boldsymbol{M}_{ab}$ là số mẫu có nhãn thật $a$ nhưng được dự đoán thành lớp $b$, $N$ là số mẫu và $\mathbb{I}(\cdot)$ là hàm chỉ thị. Các phần tử trên đường chéo là dự đoán đúng, còn các phần tử ngoài đường chéo biểu thị các cặp lớp bị nhầm lẫn.
...
\begin{equation}\operatorname{AUROC}_{\mathrm{OOD}}=\Pr\left[s(x_{\mathrm{OOD}})>s(x_{\mathrm{ID}})\right]+\frac{1}{2}\Pr\left[s(x_{\mathrm{OOD}})=s(x_{\mathrm{ID}})\right].\label{eq:ch4_auroc_ood}\end{equation}

Trong Công thức \ref{eq:ch4_auroc_ood}, $x_{\mathrm{OOD}}$ và $x_{\mathrm{ID}}$ là các mẫu được lấy lần lượt từ $\mathcal{D}_{\mathrm{OOD}}$ và $\mathcal{D}_{\mathrm{ID}}$, $s(\cdot)$ là hàm điểm bất thường và $\Pr(\cdot)$ là xác suất của quan hệ xếp hạng.
...
\begin{equation}\operatorname{AUPR\text{-}Out}=\sum_{k=1}^{K}\left(R_{k}^{\mathrm{OOD}}-R_{k-1}^{\mathrm{OOD}}\right)P_{k}^{\mathrm{OOD}}.\label{eq:ch4_aupr_out}\end{equation}

Trong Công thức \ref{eq:ch4_aupr_out}, $K$ là số điểm ngưỡng trên đường precision--recall, $P_{k}^{\mathrm{OOD}}$ và $R_{k}^{\mathrm{OOD}}$ lần lượt là precision và recall của lớp OOD tại điểm thứ $k$, còn $R_{0}^{\mathrm{OOD}}=0$ là mức recall khởi đầu.
...
\begin{equation}\tau_{95}=\inf\left\{\tau:\frac{1}{N_{\mathrm{ID}}}\sum_{x\in\mathcal{D}_{\mathrm{ID}}}\mathbb{I}\left[s(x)\leq\tau\right]\geq0{,}95\right\}.\label{eq:ch4_fpr95_threshold}\end{equation}

Trong Công thức \ref{eq:ch4_fpr95_threshold}, $\tau_{95}$ là ngưỡng nhỏ nhất đạt ID-TPR ít nhất $0{,}95$, $N_{\mathrm{ID}}$ là số mẫu ID, $\mathcal{D}_{\mathrm{ID}}$ là tập ID, $s(x)$ là điểm bất thường và $\mathbb{I}(\cdot)$ là hàm chỉ thị của quyết định chấp nhận ID.

\begin{equation}\operatorname{FPR@95\%TPR}=\frac{1}{N_{\mathrm{OOD}}}\sum_{x\in\mathcal{D}_{\mathrm{OOD}}}\mathbb{I}\left[s(x)\leq\tau_{95}\right].\label{eq:ch4_fpr95}\end{equation}

Trong Công thức \ref{eq:ch4_fpr95}, $N_{\mathrm{OOD}}$ là số mẫu OOD, $\mathcal{D}_{\mathrm{OOD}}$ là tập OOD, $s(x)\leq\tau_{95}$ biểu thị một mẫu OOD bị chấp nhận nhầm như ID và $\tau_{95}$ là ngưỡng được xác định trong Công thức \ref{eq:ch4_fpr95_threshold}.
...
\textbf{Dữ liệu và độ đo:} Bốn mô hình được đánh giá độc lập trên tập kiểm thử của BTXRD và CTCH; các tập huấn luyện và validation không được sử dụng để cập nhật mô hình. Các độ đo báo cáo gồm Accuracy, Balanced Accuracy, Macro-F1, Macro-AUROC, Macro-AUPRC và ECE theo định nghĩa tại Mục \ref{sec:evaluation_metrics}; Confusion Matrix được sử dụng để phân tích các cặp lớp thường bị nhầm lẫn. Do suy luận zero-shot không có biến thiên từ quá trình huấn luyện, khoảng tin cậy bootstrap 95\% với $B=10.000$ lần lấy mẫu lại được dùng để mô tả độ bất định do tập kiểm thử hữu hạn.
...
\textbf{Dữ liệu và độ đo:} Tất cả ba thiết lập được thực hiện trên BTXRD và CTCH với các tập huấn luyện, validation và kiểm thử cố định. Kết quả được tổng hợp trên ba hạt giống 42, 123 và 456 bằng Accuracy, Balanced Accuracy, Macro-F1, Macro-AUROC, Macro-AUPRC và ECE; Macro-F1 là độ đo dùng để lựa chọn checkpoint trên tập validation, còn Confusion Matrix hỗ trợ phân tích sai số theo cặp lớp. Khoảng tin cậy bootstrap 95\% với $B=10.000$ được tính trên tập kiểm thử. Nhằm mô tả đánh đổi giữa hiệu năng dự đoán và chi phí suy luận, tổng số và số tham số có thể huấn luyện, GFLOPs trên mỗi mẫu, thông lượng, độ trễ suy luận và bộ nhớ GPU cực đại cũng được báo cáo.
